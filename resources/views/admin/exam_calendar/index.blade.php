...
{{-- create calendar --}}
                ...
                <div class="d-flex align-items-center justify-content-center min-vh-100 px-4 pt-4 pb-5 text-center">
                    ...
                    <div
                        ...
                        <div class="bg-gray-100 px-4 py-3 sm:px-6">
                            <h3 class="text-lg font-medium text-gray-900">Ajouter une Calendrier d'examen</h3>
                        </div>

                        <!-- Modal Body -->
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <!-- Your form content goes here -->
                            <form action="{{ route('admin.calendars.create') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @if ($errors->any())
                                ...
                                <!-- File upload input -->
                                <div class="mb-4">
                                    <label for="file" class="block text-sm font-medium text-gray-700">Calendrier d'examen (PDF)</label>
        ...
        <div>
            ...
            @if(session('success'))
            <x-jet-bar-alert type="success" text="Calendrier d'examen ajoutée avec succès !" />

            @endif

            @if(session('warning'))
            <x-jet-bar-alert type="warning" text="Calendrier d'examen modifiée avec succès !" />

            @endif


            @if(session('danger'))
            <x-jet-bar-alert type="danger" text="Calendrier d'examen supprimé !" />
            ...
            <x-jet-bar-table :headers="['ID','Created_at','', '','']" class="flex justify-center">
                <template x-data="{ total:1 }"  x-for="index in total">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <x-jet-bar-table-data >
                            ...
                            <!-- Add a link to download the calendar -->

                            <a href="{{ route('admin.calendars.download', ['fileName' => $calendar->file]) }}" target="_blank" class="inline-block bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600">Télécharger</a>
...
{{-- edit calendar --}}
<div x-data="{ editModal: {{ $errors->any() && session('edit_calendar_id') == $calendar->id && session('form_type') === 'edit' ? 'true' : 'false' }}, editcalendarId: {{ $calendar->id }} }">
    <!-- Button to open modal -->
    ...
    <div x-show="editModal" class="fixed z-10 inset-0 overflow-y-auto">
        <div class="d-flex align-items-center justify-content-center min-vh-100 px-4 pt-4 pb-5 text-center">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <!-- Modal Header -->
                <div class="bg-gray-100 px-4 py-3 sm:px-6">
                    <h3 class="text-lg font-medium text-gray-900">Modifier Calendrier d'examen</h3>
                </div>

                <!-- Modal Body -->
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <!-- Your form content goes here -->
                    <form action="{{ route('admin.calendars.update', ['id'=>$calendar->id]) }}" method="POST" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        @if ($errors->any())
                        <div class="alert alert-danger">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                <input id="oldFileName" type="text" name="oldFileName" value="{{ $calendar->file ? $calendar->file : 'No file selected' }}" class="form-control" readonly>
...
<script>
    function updateFileName(event) {
        ...
        document.getElementById('oldFileName').value = fileName;
                        ...
                        <x-jet-bar-table-data>
                            <a href="#" class="text-gray-400 hover:text-gray-500">
                                <div x-data="{ deleteModal: false, calendarId: {{ $calendar->id }} }">
                                    ...
                                    <div x-show="deleteModal" class="fixed z-10 inset-0 overflow-y-auto">
                                        <div class="d-flex align-items-center justify-content-center min-vh-100 px-4 pt-4 pb-5 text-center">
                                            ...
                                            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                                                ...
                                                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                                    <form :action="`{{ route('admin.calendars.destroy', ['id'=>$calendar->id]) }}`" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <p class="text-sm text-gray-500">Etes-vous sûr que vous voulez supprimer cette calendrier d'examens?</p>
